...
def compare_user():
    ...
    # Tables de details
    df_sso_all = pd.read_sql("SELECT * FROM sso_user_detail", conn)
    df_idocs_all = pd.read_sql("SELECT * FROM idocs_user_detail", conn)
    
    # Tables users pour recuperer les sap_princ
    df_sso_users = pd.read_sql("SELECT DISTINCT id_cree, sap_princ FROM sso WHERE id_cree IS NOT NULL", conn)
    df_idocs_users = pd.read_sql("SELECT DISTINCT id_cree, sap_princ FROM idocs_user WHERE id_cree IS NOT NULL", conn)
    
    # Table des noms de concessions
    df_con_nom = pd.read_sql("SELECT sap_princ, sap_nom FROM con_nom", conn)
    
    conn.close()

    df_sso = df_sso_all[df_sso_all["id_cree"].apply(normalize_id) == id_norm]
    df_idocs = df_idocs_all[df_idocs_all["id_cree"].apply(normalize_id) == id_norm]
    ...
    # Chercher d'autres utilisateurs avec les memes IWU IDs
    if current_iwu_ids:
        for iwu_id in current_iwu_ids:
            # Dans SSO
            sso_matches = df_sso_all[
                (df_sso_all["iwu_id"] == iwu_id) & 
                (df_sso_all["id_cree"].apply(normalize_id) != id_norm)
            ]
            for _, row in sso_matches.iterrows():
                dup_id = row["id_cree"]
            ...
            # Dans IDOCS
            idocs_matches = df_idocs_all[
                (df_idocs_all["iwu_id"] == iwu_id) & 
                (df_idocs_all["id_cree"].apply(normalize_id) != id_norm)
            ]
            for _, row in idocs_matches.iterrows():
                dup_id = row["id_cree"]
                if not any(d["id_cree"] == dup_id for d in duplicates):
                    duplicates.append({
                        "id_cree": dup_id,
